...
def speech_signal() -> np.ndarray:
    test_signal = []
    with open(os.path.join(get_package_path().replace('/tqwt_tools/', '/'), 'tests', 'resources', 'speech1.txt'), 'r') \
...
def music_signal(start_sample: int = 0, end_sample: int = -1) -> np.ndarray:
    """fs = 8820Hz"""
    _, audio = read(os.path.join(get_package_path().replace('/tqwt_tools/', '/'), 'tests', 'resources',
                                 'testaudio.wav'))
