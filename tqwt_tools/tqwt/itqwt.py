...
def itqwt(w: np.ndarray, q: float, redundancy: float, n: int) -> np.ndarray:
    ...
    return np.real_if_close(
        ifft(y) * np.sqrt(y.shape[0])
    )  # inverse unitary DFT, discard small imaginary part


def synthesis_filter_bank(
    lp_subband: np.ndarray, hp_subband: np.ndarray, n: int
) -> np.ndarray:
    """
    ...
    y0[n - p - t - s : n - p - t] = np.zeros(
        (n - p - t) - (n - p - t - s)
    )  # stop-band (negative frequency)
    y0[n - p - t : n - p] = lp_subband[n0 - p - t : n0 - p] * np.flip(
        trans
    ...
    y1[n - p - t - s - 1 : n - p - t] = hp_subband[
        n1 - t - s - 1 : n1 - t
    ]  # passband (negative frequency)
    y1[n - p - t : n - p] = (
        hp_subband[n1 - t : n1] * trans
    )  # transition-band (negative frequency)
    y1[n - p : n] = np.zeros(p)
